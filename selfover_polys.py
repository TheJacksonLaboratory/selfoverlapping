...
def _invalidity_condition_1(a1, a2, vert_info):
    ...
    a3 = _get_cut(a1, vert_info, next_cut=True, same_ray=True, sign=-1)

    if a3 is None:
        return None

    if not _check_adjacency(a3, a1, vert_info, left2right=True):
        return None
...
def _invalidity_condition_2(a1, a2, vert_info):
    ...
    a3 = _get_cut(a2, vert_info, next_cut=False, same_ray=True, sign=-1)

    if a3 is None:
        return None

    if not _check_adjacency(a2, a3, vert_info, left2right=True):
        return None
...
def _invalidity_condition_3(a1, a2, vert_info, tolerance=1e-4):
    ...
    b2 = _get_cut(a1, vert_info, next_cut=True, same_ray=False, sign=0)
    b1 = _get_cut(a2, vert_info, next_cut=False, same_ray=False, sign=0)

    if b1 is None or b2 is None:
        return None

    if not (_check_adjacency(b1, a2, vert_info, left2right=True) and
            _check_adjacency(a1, b2, vert_info, left2right=True)):
        return None
    ...
    if is_valid is None:
        ...
        if check_left:
            for condition in all_invalid_conditions:
                resp = condition(a1, a2, vert_info)
                if resp is not None:
                    is_valid &= resp
                if not is_valid:
                    break
...
def _check_clockwise(vertices):
    ...
    """
    signed_area = np.sum(vertices[:-1, 0] * vertices[1:, 1] - vertices[:-1, 1] * vertices[1:, 0]) + vertices[0, 0] * vertices[-1, 1] - vertices[0, 1] * vertices[-1, 0]
...
def subdivide_selfoverlapping(vertices):
    ...
    for poly_ids in sub_polys_ids:
        new_sub_poly = vert_info[poly_ids, :2]
        if not is_clockwise:
            new_sub_poly[::-1, :]
